...
let xBtn = document.querySelector(".x-mark"); //.x-mark is class for the cross(close) button in the index.html
...
function reset(btn, song) {
  // reset is the function that will be called when the song ends
  if (btn.classList.contains("playing")) {
    // if the button has the class playing then remove the class playing
...
function progress(btn, song) {
  // progress is the function that will be called when the song is playing
  setTimeout(function () {
    // setTimeout is used to call the function after a certain time
    ...
    if (current == end) {
      // if the current time of the song is equal to the duration of the song then call the reset function
    ...
    btn.style.boxShadow =
      ...
      "px 0px 0px 0px rgba(0,0,0,0.125)"; // set the box shadow of the button
    //call function again
...
codeXamBtn.addEventListener("click", function (e) {
...
xBtn.addEventListener("click", function (e) {
  // add an event listener to the button
  e.preventDefault();
  codeXamBtn.classList.remove("is--playing");
  codeXam.pause();
  //pause the music
});
