...
function init() {
  // Create a scene and a camera let width = window.innerWidth;
...
function render() {
  ...
  if (intersects.length > 0) {
    // Get the face that is being intersected
    ...
    plane.geometry.faces.forEach(function (face) {
      // Animation loop function face.color.r = faceBaseColor.r + Math.sin(timer) * 10;
      face.color.r *= 255;
      face.color.g *= 255;
      face.color.b *= 255;
...
$(".shift-camera-button").click(function () {
  ...
  introTimeline.add([
    TweenLite.fromTo(
      ...
      0.5,
      { opacity: 1 },
      { opacity: 0, ease: Power3.easeIn, display: "none" }
    ), // Hide intro container and show sky container with animation
...
$(".x-mark").click(function () {
  // Hide sky container and show intro container with animation
